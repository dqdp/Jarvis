...
revision = "0014_loop_selection_events"
down_revision = "0013_events_truncate_guard"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    previous = tuple(
        value for value in EVENT_TYPES if not value.startswith("request.loop_selection.")
    )
    op.drop_constraint("events_event_type_check", "events", type_="check")
    op.create_check_constraint(
        "events_event_type_check",
